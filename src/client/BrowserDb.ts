...
export default class BrowserDb {
  ...
  constructor(schema: DbSchema) {
    ...
    this.open = new Promise<IDBDatabase>((resolve, reject) => {
      ...
      req.onblocked = () =>
        reject(
          new Error("Blocked. Yout should probably refresh your browser.")
        );
    });
  }
   ...
   * @param key
   */
  put(collection: string, record: any, key?: IDBValidKey) {
    return this.open.then(
      (db) =>
        new Promise<IDBValidKey>((resolve, reject) => {
          const req = db
   ...
   * @param key
   */
  delete(collection: string, key: string | number) {
    return this.open.then(
      (db) =>
  ...
  query(
    collection: string,
    where: DbFilter = null,
    orderBy: DbOrderBy = null,
    offset: number = 0,
    limit: number = 0
  ): Promise<any[]> {
    return this.all(collection).then((all: any[]) => {
      if (where) all = all.filter((rec) => where.filterRecord(rec));
      if (orderBy) orderBy.sort(all);
      if (offset < 1 && limit < 1) return all;
      else {
        const result = [];
        let end = offset + limit;
        if (limit === 0 || end > all.length) end = all.length;
  ...
  first(
    ...
    return this.query(collection, where, orderBy, 0, 1).then((arr) =>
